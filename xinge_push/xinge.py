...
class XingeHelper(object):
    ...
    def GenSign(cls, path, params, secretKey):
        ...
        return hashlib.md5(signSource.encode("utf-8")).hexdigest()
    ...
    def Request(cls, path, params):
        httpClient = http.client.HTTPConnection(cls.XINGE_HOST, cls.XINGE_PORT, timeout=cls.TIMEOUT)
        if cls.HTTP_METHOD == 'GET':
            ...
            httpClient.request(cls.HTTP_METHOD, path, urllib.parse.urlencode(params), headers=cls.HTTP_HEADERS)
        else:
            # invalid method
